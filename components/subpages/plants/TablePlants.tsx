...
export default function TableCompanies() {
  ...
  const updateEntity = async (updatedData) => {
    try {
      await axios.put(`/api/plants/${updatedData.id}`, updatedData)
      location.reload()
  ...
  const deleteEntity = async (deletedData) => {
    try {
      await axios.delete(`/api/plants/${deletedData.id}`)
      location.reload()
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-[#07314a] text-white">
            <tr>
              ...
              <th className="w-3/12 px-2 py-3 text-left xs:text-[13px] text-base md:text-lg font-medium">Location ID</th>
          ...
          <tbody className="bg-white divide-y divide-gray-300">
            {entities.map((entity) => (
              <tr key={entity.id}>
                ...
                <td className="px-2 py-4 xs:text-[13px] text-base md:text-lg font-medium">
                  <input type="text" placeholder={entity.location_id}  onChange={(e) => {
                    entity.location_id = e.target.value
                  }} />
                </td>
