...
interface RegisterPopupProps {
  isOpen: boolean;
  handlerMachine: (data:any) => void;
  onClose?: () => void;
}

const FiltersPopup: React.FC<RegisterPopupProps> = ({ isOpen, onClose, handlerMachine }) => {
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 px-5">
      <div className="bg-white rounded-lg p-6 w-[600px]">
        ...
        <div className="flex justify-end mt-20 md:mt-7">
          <button onClick={testeHandle} className="ease-in duration-300 bg-[#cfe600] text-[#07314a] py-2 px-10 rounded-xl">
            Save
          </button>
        </div>
      </div>
